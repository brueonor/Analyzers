...
namespace TestsClassAnalyzer
{
    ...
    public class TestsClassAnalyzerAnalyzer : DiagnosticAnalyzer
    {
        public const string DiagnosticId = "TestsClassAnalyzer";
        ...
        private static void AnalyzeSymbol(SymbolAnalysisContext context)
        {
            ...
            SyntaxNode syntaxRoot = namedTypeSymbol.DeclaringSyntaxReferences.First().SyntaxTree.GetRoot();
            var usings = syntaxRoot.DescendantNodes().OfType<UsingDirectiveSyntax>().ToList();

            if(usings.Any(x => x.Name.ToString().Contains("Microsoft.VisualStudio.TestTools.UnitTesting")) && !namedTypeSymbol.Name.EndsWith("Tests"))
            {
